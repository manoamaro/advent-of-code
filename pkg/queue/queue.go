...
// SeqLeft returns an iterator that yields elements from the queue until it is empty, popping elements from the left as it goes.
func (q *Queue[T]) SeqLeft() iter.Seq[T] {
	return func(yield func(T) bool) {
		for len(*q) > 0 {
			if !yield(*q.PopLeft()) {
				return
